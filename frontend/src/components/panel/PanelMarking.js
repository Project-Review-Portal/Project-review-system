...
const PanelMarking = () => {
    ...
    const fetchSettingsAndData = async () => {
        ...
        try {
            ...
            if (dynamicSlots.length > 0) {
                setActiveSlotType(prev => dynamicSlots.includes(prev) ? prev : dynamicSlots[0]);
            }
    ...
    // When activeSlotType or teams change, fetch schemes for all teams that have a panel
    useEffect(() => {
    ...
    const handleSubmitTeamMarks = async (team, schemeComps) => {
        try {
            ...
            const requests = students.map(stu => {
                ...
                return axios.post(`${SERVER_API_KEY}/api/panels/marks`, {
                    teamId: team._id,
                    studentId,
    ...
    if (loading) return (
        <div className="flex items-center justify-center p-16">
            ...
            <span className="ml-3 text-gray-600 font-medium">Loading panel marking data...</span>
        </div>
    );
    ...
    return (
        <div className="space-y-8">
            <div className="bg-white p-6 rounded-lg shadow">
                ...
                <p className="text-sm text-gray-500 mt-1">Enter and submit evaluation marks for teams assigned to your panel. Mark columns are defined by the coordinator's marking scheme.</p>
            </div>
            ...
            {teams.length === 0 ? (
                <div className="p-8 bg-white rounded-lg shadow text-center text-gray-500 font-medium">
                    No teams assigned to your panel for evaluation.
                </div>
            ) : (
                teams.map(team => {
                            ...
                            <div className="flex items-center gap-3 flex-wrap">
                                ...
                                {panelId && (
                                    <span className="px-2.5 py-0.5 text-xs font-medium rounded-full bg-gray-100 text-gray-600">
                                        Panel: {team.panel?.name || team.panelName || 'Assigned'}
                                    </span>
                                )}
                            </div>
